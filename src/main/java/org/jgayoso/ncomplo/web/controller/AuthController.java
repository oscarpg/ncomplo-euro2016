...
public class AuthController {
  ...
  public String processInvitationGroup(
      @PathVariable("leagueToken") final String token,
      final ModelMap model,
      final RedirectAttributes redirectAttributes) {
  ...
  public String processInvitation(
    ...
    final Invitation invitation = this.invitationService.findById(invitationId);
    if (invitation == null
        || !invitation.getLeague().getId().equals(leagueId)
        || !invitation.getEmail().startsWith(emailId)) {
      logger.info("Invalid invitation " + leagueId + " for values " + invitationId + ", " + leagueId + ", " + emailId);
      redirectAttributes.addFlashAttribute("error", "Invalid invitation");
      return "redirect:/login?error";
    }
  ...
  public String acceptInvitation(
    ...
    try {
      this.userService.registerFromInvitation(
          invitationId, userBean.getLogin(), userBean.getName(), userBean.getEmail(), leagueId, userBean.getPassword());
    } catch (final LeagueClosedException e) {
      redirectAttributes.addFlashAttribute("error", "League is closed");
      return "redirect:/login";
  ...
  public String acceptInvitationGroup(final UserInvitationBean userBean, final RedirectAttributes redirectAttributes) {

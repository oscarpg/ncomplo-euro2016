...
public class LeagueService {
  ...
  public void recomputeScores(final Integer leagueId) {
    ...
    for (final User participant : league.getParticipants()) {
      final List<Bet> bets = this.betRepository.findByLeagueIdAndUserLogin(league.getId(), participant.getLogin());
  ...
  public void recomputeScoresForGames(final Integer leagueId, final Collection<Game> games) {
    ...
    for (final User participant : league.getParticipants()) {
      final List<Bet> bets =
          this.betRepository.findByLeagueIdAndUserLoginAndGameIn(league.getId(), participant.getLogin(), games);
      for (final Bet bet : bets) {
        bet.evaluate();
      }
    }
  ...
  private TodayRoundGamesAndBetsView processScoreMattersGames(
      final Integer leagueId, final Round round, final List<Game> games) {
  ...
  private TodayRoundGamesAndBetsView processSideMattersGames(
      final Integer leagueId, final Round round, final List<Game> games) {
    ...
    final TodayRoundGamesAndBetsView betsView =
        new TodayRoundGamesAndBetsView(round, games, null, sideMatterBets, false);
    betsView.setMaxSideMattersBets(maxNumber);
    return betsView;
  }
